...
set $NR_THREADS=4
set $LAZYFS_FIFO="/tmp/lfs.fb2.rand-write.32768.fifo"

set $NR_FILES=1
set $MEAN_DIR_WIDTH=1
set $IO_SIZE=4k
set $FILE_SIZE=16g
set $NR_ITERATIONS=4194304
...
define process name="process1", instances=1
{
    thread name="thread1", memsize=10m, instances=1
    {
        ...
        flowop write name="write1", fd=1, iosize=$IO_SIZE, iters=$NR_ITERATIONS, random
    ...
    thread name="thread2", memsize=10m, instances=1
    {
        ...
        flowop write name="write2", fd=1, iosize=$IO_SIZE, iters=$NR_ITERATIONS, random
    ...
    thread name="thread3", memsize=10m, instances=1
    {
        ...
        flowop write name="write3", fd=1, iosize=$IO_SIZE, iters=$NR_ITERATIONS, random
    ...
    thread name="thread4", memsize=10m, instances=1
    {
        ...
        flowop write name="write4", fd=1, iosize=$IO_SIZE, iters=$NR_ITERATIONS, random
