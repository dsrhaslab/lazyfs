...
set $NR_THREADS=4
set $LAZYFS_FIFO="/tmp/lfs.fb2.seq-read.131072.fifo"
...
define process name="process1", instances=1
{
    thread name="thread1", memsize=10m, instances=1
    {
        ...
        flowop read name="read1", fd=1, iosize=$IO_SIZE, iters=$NR_ITERATIONS
    ...
    thread name="thread2", memsize=10m, instances=1
    {
        ...
        flowop read name="read2", fd=1, iosize=$IO_SIZE, iters=$NR_ITERATIONS
    ...
    thread name="thread3", memsize=10m, instances=1
    {
        ...
        flowop read name="read3", fd=1, iosize=$IO_SIZE, iters=$NR_ITERATIONS
    ...
    thread name="thread4", memsize=10m, instances=1
    {
        ...
        flowop read name="read4", fd=1, iosize=$IO_SIZE, iters=$NR_ITERATIONS
